...
function formatOperationText(data) {
  return `${data.CUIT}|${dayjs().format('YYYYmmdd')}|${dayjs().format('HHmmss')}|${data.operationType === 0 ? 'C' : 'V' }|${(data.token).padEnd(30)}|${(String(data.amount)).padEnd(10)}|${(String(Number(data.price/100).toFixed(2))).padEnd(10)|('100.50').padEnd(10)};`;//format the output to the file
}
