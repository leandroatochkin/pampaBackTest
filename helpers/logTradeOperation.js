...
function formatOperationText(data) {
  return `${data.CUIT}|${dayjs().format('YYYYMMDD')}|${dayjs().format('HHmmss')}|${data.operationType === 0 ? 'C' : 'V' }|${String(data.symbol).padStart(3, '0')}|${String(data.tokenName).padEnd(30)}|${String(data.amount).slice(0, 10).padEnd(10, ' ')}|${(String(Number(data.price/100).toFixed(2))).padEnd(12)}|${('100.50').padEnd(12)};`;//format the output to the file
}
